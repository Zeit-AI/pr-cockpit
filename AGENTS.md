...
This is Zeit AI's fork. Read `FORK.md` first: how to change it without making the next rebase expensive, and the traps that already cost us time.
...
- Server startup marks every still-pending mutation `failed: interrupted` rather than resending blindly; after hand-repairing a queued row and restarting, resend it with `POST /api/mutations/<id>/retry`.
